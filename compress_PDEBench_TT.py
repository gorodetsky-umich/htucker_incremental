...
from compress_PDEBench_HT import get_args, read_snapshot, initialize_wandb
...
def main():
    ...
    if args.wandb:
        ...
        wandb.init(
            ...
            name="PDEBench_TT_eps_"+"".join(f"{args.epsilon:0.2f}_".split("."))+f"batchsize_{args.batch_size:04d}_"+"shape_"+"_".join(map(str,args.reshaping))+"_date_"+timestamp,
            config=run_config,
...
def numpy_combine(args):
    ...
    if args.wandb:
        initialize_wandb(args,timestamp, method="TT", tags = ["PDEBench", "TT-ICE", MACHINE_ALIAS, args.M, args.type])
    ...
    print(len(train_simulations),len(val_simulations),len(test_simulations),)
    train_batch = []
    ...
    while train_simulations[:]:
        ...
        # print(train_batch.shape[-1],rec.shape, nla.norm(rec),dataset.ttCores[-1][:,-train_batch.shape[-1],:].shape)
        # print(len(dataset.normalizing_constants),dataset.ttCores[-1].shape[1])
        error_after_update = nla.norm(rec-train_batch)/batch_norm
        total_time += batch_time
