...
def main():
    parser = argparse.ArgumentParser(description='This script reads the BigEarthNet image patches and writes them into NumPy files')
    parser.add_argument('-d', '--data_location', dest='data_location', help='path to data', default=None)
    parser.add_argument('-s', '--save_location', dest='save_location', help='path to save the NumPy files', default=None)
    ...
    if args.save_location is None:
        ...
        save_loc = reduce(os.path.join,[PATH_SEP]+HOME.split(os.path.sep)+["data","BigEarthNet-v1.0_numpy"])
    else:
    ...
    image_size = cv2.imread(image_folders[0]+PATH_SEP+os.listdir(image_folders[0])[0], cv2.IMREAD_UNCHANGED).shape

    for band in BAND_NAMES:
        f = rasterio.open(image_folders[0]+PATH_SEP+image_folders[0].split(PATH_SEP)[-1]+f"_{band}.tif")
    ...
    # for file in tqdm.tqdm(glob.glob(HOME+"/data/BigEarthNet-v1.0_numpy/*.npy")):
    #     anan = np.load(file)
