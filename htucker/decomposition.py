...
import numpy as np
from htucker.utils import create_permutations, mode_n_unfolding, mode_n_product


def truncated_svd(a, truncation_threshold=None, full_matrices=True, compute_uv=True, hermitian=False):
    """Truncated SVD computation with error control.
...
def hosvd_only_for_dimensions(tensor, tol=None, rtol=None, threshold=1e-8, norm=None, 
                             dims=None, batch_dimension=None, contract=False):
    ...
    Returns:
        list: Left singular vectors or (contracted tensor, left singular vectors) if contract is True
    """
    if dims is None:
