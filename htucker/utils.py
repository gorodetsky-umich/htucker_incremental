...
class NotFoundError(Exception):
    ...
    def __init__(self, value):
        """Initialize the NotFoundError with a message."""
        self.value = value

    def __str__(self):
        """Represent the error message in string form."""
        return repr(self.value)


def create_permutations(dims):
    """Create permutations to compute the matricizations.
...
def mode_n_unfolding(tensor, mode):
    """Compute mode-n unfolding/matricization of a tensor in the sense of Kolda&Bader.
...
def mode_n_product(tensor, matrix, modes):
    ...
    dims = list(range(len(tensor.shape) + len(matrix.shape) - 2))
    # tensor_ax, matrix_ax = mode, 1  # Mode axis of tensor, second axis of matrix
    tensor_ax, matrix_ax = modes  # Mode axis of tensor, second axis of matrix
    dims.pop(tensor_ax)
