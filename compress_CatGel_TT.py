...
def compress_catgel_full_sim_TT(args):
    ...
    if args.wandb:
        initialize_wandb_catgel(args,timestamp,tags=['CatGel','TT-ICE',MACHINE_ALIAS],method="TT")
    ...
    rec = dataset.reconstruct(
        dataset.ttCores[-1][:,-training_snapshot.shape[-1]:,:]
    ).squeeze(0)[...,0]
    ...
    print(f"{batch_index:04d} {dataset.ttCores[-1].shape[1]:06d} {round(batch_time, 5):09.5f} {round(total_time, 5):10.5f} {batch_norm:14.5f} {round(error_before_update, 5):0.5f} {round(error_after_update, 5):0.5f} {round(dataset.compressionRatio, 5):09.5f} {round(np.mean(test_errors),5):0.5f} {' '.join(map(lambda x: f'{x:03d}', ranks))}")
