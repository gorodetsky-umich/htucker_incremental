...
def initialize_wandb(args,timestamp,method,tags):
    ...
    wandb.init(
        ...
        name=f"PDEBench_{method}_eps_"+"".join(f"{args.epsilon:0.2f}_".split("."))+f"{args.normalization}_batchsize_{args.batch_size:04d}_"+"shape_"+"_".join(map(str,args.reshaping))+"_states_"+"_".join(map(str,args.states))+"_date_"+timestamp,
        config=run_config,
        tags=tags,
    )
...
# def test_projection_error(simulation,num_timesteps,states,data_loc):
    
#     ## read data
#     # test_sim = np.zeros()
#     with Pool() as pool:
#         test_sim = pool.map(partial(read_snapshot,data_loc=data_loc,simulation=simulation,states=states),list(range(num_timesteps)))
#     print(np.array(test_sim).shape)
#     # for timestep in range(num_timesteps):
#     #     train_batch[:,:,:,timestep,:,sim_idx] = np.load(data_loc+f'sim{simulation:03d}_ts{timestep:02d}.npy')[:,:,:,args.states]
#     ## project data
...
def numpy_combine(args):
    ...
    if args.transpose is not None:
        assert len(args.transpose) == (len(args.reshaping)+3), f"Transpose dimensions {args.transpose} are not in aggreement with the shape of data {args.reshaping+[num_states,num_timesteps,args.batch_size]}"

    if args.wandb:
        initialize_wandb(args, timestamp, method="HT", tags = ["PDEBench", "HTucker", MACHINE_ALIAS, args.M, args.type])
    ...
    total_time = 0
    batch_index = 0
    print(f"Number of training simulations: {len(train_simulations)}\nNumber of validation simulations:{len(val_simulations)}\nNumber of testing simulations:{len(test_simulations)}")
    # print(train_batch.shape)
    # print()
    # Note to self: you can parallelize this part if needed (I don't think it will be needed but anyway)
    tic = time.time()
    train_batch = []
    ...
    train_batch = train_batch.reshape(args.reshaping+[num_states,num_timesteps,args.batch_size], order=ORD)
    if args.transpose:
        train_batch = np.transpose(train_batch,args.transpose)
    print(train_batch.shape)
    ...
    for simulation in val_simulations:
        with Pool() as p:
            val_data  = p.map(partial(read_snapshot,data_loc=data_loc,simulation=simulation,states=args.states),list(range(num_timesteps)))
        # val_data = np.concatenate(val_data,axis=-1).transpose(0,1,2,4,3)[...,None]
        val_data = np.concatenate(val_data,axis=-1)
        for state_idx in range (num_states):
            val_data[..., state_idx,:], _ , _ = normalize(val_data[..., state_idx,:], method = args.normalization) 
    ...
    for simulation in test_simulations:
        with Pool() as p:
            test_data  = p.map(partial(read_snapshot,data_loc=data_loc,simulation=simulation,states=args.states),list(range(num_timesteps)))
        test_data = np.concatenate(test_data,axis=-1)
        for state_idx in range (num_states):
            test_data[..., state_idx,:], _ , _ = normalize(test_data[..., state_idx,:], method = args.normalization) 
    ...
    while train_simulations[:]:
        ...
        for simulation in train_simulations[:args.batch_size]:
            ...
            # train_batch.append(np.concatenate(sim,axis=-1).transpose(0,1,2,4,3)[...,None])
        train_batch = np.concatenate(train_batch,axis=-1)
        # print(len(train_simulations),train_batch.shape)
        # train_batch = train_batch.reshape(args.reshaping+[num_timesteps,num_states,args.batch_size], order=ORD)
        train_batch = train_batch.reshape(args.reshaping+[num_states,num_timesteps,args.batch_size], order=ORD)
        if args.transpose:
            train_batch = np.transpose(train_batch,args.transpose)
