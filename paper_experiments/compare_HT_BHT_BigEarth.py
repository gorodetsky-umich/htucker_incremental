...
def initalize_wandb(args, timestamp, tags=[]):
    ...
    wandb.init(
        ...
        name=f"BigEarthNet_{args.method}_eps_"
...
def hierarchical_tucker_bigearth(args):
    ...
    for k in range(5000):  # This will have the experiment repeat until 5000 batches => 500k frames
        ...
        compressed_batch = []
        try:
            del dataset  # noqa: F821
        except Exception:
            pass

        train_batch = np.zeros(
            args.reshaping + [(k + 1) * args.batch_size],
        )
        ...
        # Calculate actual error metric
        numerator = np.square(metrics[:, 2]).sum()
        denominator = np.square(metrics[:, 1]).sum()
        actual_error = np.sqrt(1 - (numerator / denominator))
        line_to_append.append(f"{round(actual_error, 5):0.5f}")
        ...
        logging_dict = {
            ...
            "actual_error": np.sqrt(
                1 - (np.square(metrics[:, 2]).sum() / np.square(metrics[:, 1]).sum())
            ),
...
if __name__ == "__main__":
    ...
    if args.wandb:
        initalize_wandb(
            args, timestamp, tags=["BigEarthNet", "HTvsBHT", args.method, MACHINE_ALIAS]
        )
