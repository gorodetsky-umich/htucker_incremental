...
PDEBENCH_STATES = ["Vx", "Vy", "Vz", "density", "pressure"]


def initialize_wandb_catgel(args, timestamp, tags: list[str], method: str):
    run_config = wandb.config = {  # type: ignore[attr-defined]
    ...
    wandb.init(  # type: ignore[attr-defined]
        ...
        name=f"CatGel_{method}_eps_"
        + "".join(f"{args.epsilon:0.2f}_".split("."))
        + f"{args.normalization}"
        + f"_type_{args.type}_batchsize_{args.batch_size:03d}"
        + "_date_"
        + timestamp,
        config=run_config,
        tags=tags,
...
def get_args_catgel():
    ...
    parser.add_argument(
        "-t", "--type", dest="type", type=int, help="Type of simulation data", default=1
    )
    parser.add_argument(
        "-n",
        "--normalization",
        dest="normalization",
        type=str,
        help="Method of normalization that is used",
        default="none",
    )
...
def get_args_pdebench():
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        help="Determines the reshaping for the tensor stream. Note that the transpose \
            operation happens before reshaping!",
        default=[],
    )
    parser.add_argument(
        "-T",
        ...
        nargs="+",
        type=int,
        help="Swaps the axes for the tensor stream",
        default=None,
    )
    parser.add_argument(
        "-d", "--data_location", dest="data_location", help="path to data", default=None
    ...
    parser.add_argument(
        ...
        default=list(range(len(PDEBENCH_STATES))),
    )
    parser.add_argument(
        "-b", "--batch_size", dest="batch_size", type=int, help="Batch size", default=1
    )
    ...
    parser.add_argument(
        ...
        dest="save",
        action="store_true",
        help="Enables saving the compression",
        default=False,
    )
    parser.add_argument(
        "-w",
...
def normalize(arr: np.ndarray, method: str):
    implemented_methods = [
        "minmax",
        "zscore",
        "maxabs",
    ...
    if method not in implemented_methods:
        raise NotImplementedError("method should be one of  " + " ".join(implemented_methods) + "!")
    if method == "none":
        return arr, 0, 1
    else:
        if method == "minmax":
