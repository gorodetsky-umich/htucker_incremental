...
from experiment_utils import normalize
...
def parser_pdebench():
    parser = argparse.ArgumentParser(
        description="This script is to compare Hierarchical Tucker decomposition \
            and Batch Hierarcical Tucker decomposition."
    )
    parser.add_argument(
...
def initialize_wandb(args, timestamp, tags=[]):
    ...
    wandb.init(
        ...
        name=f"PDEBench_{args.method}_eps_"
...
def hierarchical_tucker_pdebench(args):
    ...
    if args.wandb:
        initialize_wandb(
            args,
            timestamp,
            tags=["PDEBench", "HTvsBHT", args.method, MACHINE_ALIAS, args.M, args.type],
        )
    ...
    for k in range(len(train_simulations) // args.batch_size):
        ...
        for simulation_idx, simulation in enumerate(train_simulations[: (k + 1) * args.batch_size]):
            ...
            for state_idx in range(num_states):
                ...
                (
                    train_batch[..., state_idx, :, simulation_idx],
                    normalizing_constant1,
                    normalizing_constant2,
                ) = normalize(sim[..., state_idx, :], method=args.normalization)
            simulationwise_sim_norm[simulation_idx] = nla.norm(train_batch[..., simulation_idx])
        train_batch = train_batch.reshape(
            args.reshaping + [num_states, num_timesteps, (k + 1) * args.batch_size], order=ORD
        )
        # print(train_batch.shape)
        # quit()
        ...
        for simulation_idx, simulation in enumerate(train_simulations[: (k + 1) * args.batch_size]):
            ...
            simulationwise_error[simulation_idx] = (
                nla.norm(train_batch[..., simulation_idx] - temp_ht.root.core.squeeze())
                / simulationwise_sim_norm[simulation_idx]
            )
        ...
        rel_error = np.sqrt(
            1
            - (
                np.square(simulationwise_approx_norm).sum()
                / np.square(simulationwise_sim_norm).sum()
            )
        )
        line_to_append.append(f"{round(rel_error, 5):0.5f}")
        ...
        with open(
            DIRECTORIES["metric"] + METRICS_FILE + f"eps_{args.epsilon}_{args.method}.txt", "a"
        ) as f:
...
def batch_hierarchical_tucker_pdebench(args):
    ...
    for k in range(len(train_simulations) // args.batch_size):
        ...
        for simulation_idx in range(dataset.root.core.shape[-1]):
            simulationwise_approx_norm[simulation_idx] = nla.norm(
                dataset.root.core[..., simulation_idx]
            )

        approx_norm = nla.norm(dataset.root.core)

        simulationwise_error = np.sqrt(
            1 - (np.square(simulationwise_approx_norm) / np.square(simulationwise_sim_norm))
        )
...
if __name__ == "__main__":
    ...
    DIRECTORIES = {
        ...
        + PATH_SEP,
        "metric": reduce(
            os.path.join, [PATH_SEP] + CWD.split(os.path.sep) + ["experiments", "PDEBench"]
        )
        + PATH_SEP,
    }
