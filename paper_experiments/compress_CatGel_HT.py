...
def compress_catgel_full_sim_HT(args):
    ...
    if args.wandb:
        initialize_wandb_catgel(
            args, timestamp, tags=["CatGel", "HTucker", MACHINE_ALIAS], method="HT"
        )
    ...
    dataset = ht.HTucker()
    dataset.initialize(training_snapshot, batch=True, batch_dimension=batch_along)
    ...
    dataset.normalizing_constants = [
        np.concatenate(normalizing_constants, axis=-1).T
    ]  # type:ignore
    ...
    for simulation in test_simulations[:]:
        ...
        rec = dataset.reconstruct(
            dataset.project(test_snapshot, batch=True, batch_dimension=batch_along)
        )
    ...
    while training_simulations[:]:
        ...
        projection = dataset.reconstruct(
            dataset.project(training_snapshot, batch=True, batch_dimension=batch_along)
        )
