...
from experiment_utils import denormalize
...
def load_simulation(
    simulation_idx, data_loc, num_timesteps=1, states=list(range(len(STATES))), sim_shape=None
):
    if sim_shape is None:
        timestep = 0
        sim_shape = list(
            np.load(data_loc + f"sim{simulation_idx:03d}_ts{timestep:02d}.npy")[:, :, :, states][
                ..., None
            ].shape
        )
...
def compute_energy_spectrum_new(data, L=1.0):
    ...
    for t in range(nsteps):
        ...
        for N in range(k_end):
            spectrum[N] = (
                np.sum(muu[inds == N + 1]) + np.sum(mvv[inds == N + 1]) + np.sum(mww[inds == N + 1])
            )
...
def get_args():
    parser = argparse.ArgumentParser(
        description="This script compares the energy spectrum of tensor network\
            compressed NS simulations with turbulent ICs from the PDEBench dataset"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-m", "--method", dest="method", type=str, help="Method of compression", default=None
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Prints the energy spectrum to a .npy file.",
    )
    parser.add_argument(
        "-N",
        "--normalization",
...
def energy_spectrum_ht(args):
    ...
    # assert np.prod(grid_shape) == np.prod(args.reshaping),
    ...
    rec_sim = dataset.reconstruct(root_slice).reshape(
        grid_shape + [num_states, num_timesteps], order=ORD
    )
    ...
    for state_idx in range(rec_sim.shape[-2]):
        ...
        rec_sim[..., state_idx, :] = denormalize(
            rec_sim[..., state_idx, :],
            args.normalization,
            normalizing_constant1,
    ...
    plt.title(
        f"Time Averaged Energy Spectrum w/ {args.epsilon*100}% Rel.Err.\
            ({round(np.prod(sim.shape)/np.prod(dataset.root.shape[:-1]),3)}x compression)"
    )
    # plt.loglog(k, spectrum)
    plt.xlabel("Wavenumber")
    plt.ylabel("Average Energy")
    ...
    test_sim = load_simulation(
        val_simulations[test_sim_idx], data_loc, num_timesteps, states=args.states
    )
    ...
    dataset = dmt.ttObject.loadData(
        compression_loc + f"PDEBench3_{args.normalization}_e{epsilon_str}.ttc"
    )
    ...
    # print(avg_spectrum.shape,spectrum.shape,k.shape)
    plt.semilogy(
        k_rec, ta_rec_spec, label=rf"TT (${round(dataset.compressionRatio,3)}\times$ compression)"
    )
    # plt.semilogy(k,ta_spec,label='original')
    plt.title(
        f"Time Averaged Energy Spectrum w/ {args.epsilon*100}% Rel.Err.\
            ({round(np.prod(sim.shape)/np.prod(dataset.ttRanks[-1]),3)}x compression)"
    )
    # plt.loglog(k, spectrum)
    plt.xlabel("Wavenumber")
    plt.ylabel("Average Energy")
    ...
    root_slice = dataset.projectTensor(
        test_sim.reshape(args.reshaping + [num_timesteps], order=ORD)
    )
    rec_sim = dataset.reconstruct(root_slice).reshape(
        grid_shape + [num_states, num_timesteps], order=ORD
    )
