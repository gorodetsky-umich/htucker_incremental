...
def main():
    parser = argparse.ArgumentParser(description='This script reads the PDEBench simulations from hdf5 files and writes them into NumPy files')
    ...
    if args.save_location is None:
        print("Save location is not provided, writing under the same directory as the input data")
        ...
        for sim_idx in tqdm(range(num_simulations)):
            for timestep in range(num_timesteps):
                for state_idx, state in enumerate(STATES):
                    ...
                    # print(f[state][sim_idx][timestep].shape)
